 ...
 */
public class PickerAlbumActivity extends UI implements OnAlbumItemClickListener, OnPhotoSelectClickListener,
        OnClickListener {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        options.logoId = 0;
        options.navigateId = R.drawable.toolbar_back_select_style;
        setToolBar(R.id.toolbar, options);
//        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {//Android 5.0及以上
//            getWindow().setStatusBarColor(ContextCompat.getColor(this, R.color.blue_2bbbe6));
//        }
    ...
    public void onPhotoSingleClick(List<PhotoInfo> photos, int position) {
        if (isMutiMode) {
            PickerAlbumPreviewActivity.start(this, photos, position, isSupportOriginal, isSendOriginalImage,
                    hasSelectList, mutiSelectLimitSize);
        } else {
            if (photos != null) {
    ...
    private void updateSelectBtnStatus() {
        ...
        if (selectSize > 0) {
            ...
            pickerSend.setText(String.format(this.getResources().getString(R.string.picker_image_send_select),
                    selectSize));
        } else {
    ...
    public void onClick(View v) {
        if (v.getId() == R.id.picker_bottombar_preview) {
            PickerAlbumPreviewActivity.start(this, hasSelectList, 0, isSupportOriginal, isSendOriginalImage,
                    hasSelectList, mutiSelectLimitSize);
