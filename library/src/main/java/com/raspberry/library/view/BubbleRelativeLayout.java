...
/**
 * Created by xiaohu on 2016/12/1.
 * <p>
 * 气泡布局
 */
...
public class BubbleRelativeLayout extends RelativeLayout {
    ...
    public static int PADDING_LEFT = 30;
    public static int PADDING_RIGHT = 30;
    public static int PADDING_BOTTOM = 30;
    ...
    public void setBubbleParams(final BubbleLegOrientation bubbleOrientation, final float bubbleOffset) {
        ...
        switch (mBubbleOrientation) {
            case LEFT:
                renderBubbleLegPrototype(PADDING_LEFT);
                MIN_LEG_DISTANCE = PADDING_LEFT + LEG_HALF_BASE;
                break;
            case TOP:
                renderBubbleLegPrototype(PADDING_TOP);
                MIN_LEG_DISTANCE = PADDING_TOP + LEG_HALF_BASE;
                break;
            case RIGHT:
                renderBubbleLegPrototype(PADDING_RIGHT);
                MIN_LEG_DISTANCE = PADDING_RIGHT + LEG_HALF_BASE;
                break;
            case BOTTOM:
                renderBubbleLegPrototype(PADDING_BOTTOM);
                MIN_LEG_DISTANCE = PADDING_BOTTOM + LEG_HALF_BASE;
                break;
            default:
                break;
        }
    ...
    public void setPathEffect(int radius) {
        CORNER_RADIUS = radius;
        mPaint.setPathEffect(new CornerPathEffect(CORNER_RADIUS));
    }

    public void renderBubbleLegPrototype(int radius) {
        mBubbleLegPrototype.moveTo(0, 0);
        mBubbleLegPrototype.lineTo(radius * 1.5f, -radius / 1.5f);
        mBubbleLegPrototype.lineTo(radius * 1.5f, radius / 1.5f);
        mBubbleLegPrototype.close();
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        mPath.rewind();
        mPath.addRoundRect(new RectF(PADDING_LEFT, PADDING_TOP, width - PADDING_RIGHT, height - PADDING_BOTTOM),
