...
import com.raspberry.library.view.ImageViewPagerAdapter;
...
public class AdViewpagerUtil {
    ...
    private boolean isAllowLoop = false;//是否允许轮播
     ...
     * @param urls
     */
    public AdViewpagerUtil(Context context, ViewPager viewPager, LinearLayout dotlayout, int dotsize, int dotoffset,
                           List<String> urls) {
        this.context = context;
        this.viewPager = viewPager;
    ...
    private void setImg(int length, List<String> urls) {
        if (urls.size() > 0) {
            ...
            for (int i = 0; i < length; i++) {
                if (i < length - 2) {
                    final int index = i;
                    Glide.with(context).load(urls.get(i)).into(mImageViews.get(i + 1));
                    mImageViews.get(i + 1).setOnClickListener(new View.OnClickListener() {
                        ...
                        public void onClick(View v) {
                            ...
                            if (onAdItemClickListener != null) {
                                onAdItemClickListener.onItemClick(mImageViews.get(index + 1), index);
                            }
                        }
                    });
                }
            }
            Glide.with(context).load(urls.get(urls.size() - 1)).centerCrop().into(mImageViews.get(0));
            Glide.with(context).load(urls.get(0)).centerCrop().into(mImageViews.get(length - 1));
        }
    }
    ...
    public void initDots(int length) {
        ...
        LinearLayout.LayoutParams mParams = new LinearLayout.LayoutParams(dip2px(context, dotsize), dip2px(context,
                dotsize));
        ...
        for (int i = 0; i < length; i++) {
            ...
            dotViews.add(imageView);//得到每个小圆点的引用，用于滑动页面时，（onPageSelected方法中）更改它们的状态。
    ...
    Runnable runnable = new Runnable() {
        ...
        public void run() {
            // 要做的事情
            if (viewPager.getChildCount() > 0 && imgsize > 0) {
    ...
    private void startLoopViewPager() {
        if (isAllowLoop && !isLoop && viewPager != null) {
