...
public class UploadHealthReportActivity extends BaseActivity implements View.OnClickListener, View
        .OnLongClickListener, View.OnFocusChangeListener, IAudioRecordCallback {

    public static final int PHYSICAL = 121;//体检报告
    public static final int HISTORY = 122;//病历报告
    ...
    private boolean isUploadAudioing = false;//是否正在上传语音文件
    private boolean isUploadPictureing = false;//是否正在上传图片文件
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        uploadType = getIntent().getIntExtra(UploadHealthReportActivity.class.getSimpleName(), -1);
        if (uploadType < 0) {
    ...
    private void initData() {
        switch (uploadType) {
            case PHYSICAL://体检报告
                toolbar = initToolbar(R.id.toolbar, R.id.toolbar_title, R.string.upload_health_physical);
                break;
            case HISTORY://病历报告
                toolbar = initToolbar(R.id.toolbar, R.id.toolbar_title, R.string.upload_health_history);
                break;
            default:
                break;
        }
        ...
        adapter = new BaseRecyclerAdapter<String>(this, datalist) {
            ...
            public void bindData(BaseRecyclerViewHolder holder, final int position, String item) {
                ...
                if (position == datalist.size() - 1) {
                    Glide.with(UploadHealthReportActivity.this).load(R.drawable.main_healthreport_upload_addphoto)
                            .into(imageView);
                        ...
                        public void onClick(View v) {
                            if (datalist.size() > MAX_NUMBER) {
                                ...
                                MultiImageSelector.create().showCamera(true).count(MAX_NUMBER).multi().origin
                                        (tempList).start(UploadHealthReportActivity.this, REQUEST_IMAGE);
                            }
                        }
                    });
                } else {
                    Glide.with(UploadHealthReportActivity.this).load(datalist.get(position)).centerCrop().into
                            (imageView);
                    holder.getImageView(R.id.multi_select_item_cancel).setVisibility(View.VISIBLE);
                    imageView.setOnClickListener(new View.OnClickListener() {
                        @Override
                        public void onClick(View view) {
                            ImagePagerActivity.startImagePagerActivity(UploadHealthReportActivity.this, tempList,
    ...
    private String cerateBodyStr() {
        String bodrStr = null;
        bodrStr = "content=" + TextUtils.getText(describeET);
        bodrStr = bodrStr + "&patientName=" + TextUtils.getText(userNameET);
        bodrStr = bodrStr + "&patientAge=" + TextUtils.getText(userAgeET);
        bodrStr = bodrStr + "&patientSex=" + Sex.getSex(TextUtils.getText(userSexTV));
        ...
        if (null != multiUrls && multiUrls.size() > 0) {
            ...
            //                bodrStr = bodrStr + "&map['picture'].content[" + i + "].address=" + multiUrls.get(i);
            //            }
            int i = 0;
    ...
    private void uploadReport() {
        ...
        ReportType type;
        if (uploadType == PHYSICAL) {
            type = ReportType.PHYSICALEXAM;
        } else {
            type = ReportType.CASEHISTORY;
        }

        HttpProtocol.addReport(cerateBodyStr(), type, callback);
    ...
    private void compress(final File file, final String url, final CountDownLatch countDownLatch) {
        ...
        Luban.get(UploadHealthReportActivity.this)//
                .load(file)//传入要压缩的图片
                .putGear(Luban.THIRD_GEAR)      //设定压缩档次，默认三挡
                .asObservable()//
