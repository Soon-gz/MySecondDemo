...
public class AVChatUI implements AVChatUIListener {
    ...
    public AVChatUI(Context context, View root, AVChatListener listener, ClientType clientType) {
        this.context = context;
        this.root = root;
        this.aVChatListener = listener;
        this.avChatOptionalConfig = new AVChatOptionalConfig();
    ...
    private void configFromPreference(SharedPreferences preferences) {
        ...
        videoQuality = Integer.parseInt(preferences.getString(context.getString(R.string
                .nrtc_setting_vie_quality_key), 0 + ""));
        serverRecordAudio = preferences.getBoolean(context.getString(R.string
                .nrtc_setting_other_server_record_audio_key), false);
        serverRecordVideo = preferences.getBoolean(context.getString(R.string
                .nrtc_setting_other_server_record_video_key), false);
        defaultFrontCamera = preferences.getBoolean(context.getString(R.string
                .nrtc_setting_vie_default_front_camera_key), true);
        autoCallProximity = preferences.getBoolean(context.getString(R.string.nrtc_setting_voe_call_proximity_key),
                true);
        videoHwEncoderMode = Integer.parseInt(preferences.getString(context.getString(R.string
                .nrtc_setting_vie_hw_encoder_key), 0 + ""));
        videoHwDecoderMode = Integer.parseInt(preferences.getString(context.getString(R.string
                .nrtc_setting_vie_hw_decoder_key), 0 + ""));
        videoFpsReported = preferences.getBoolean(context.getString(R.string.nrtc_setting_vie_fps_reported_key), true);
        audioEffectAecMode = Integer.parseInt(preferences.getString(context.getString(R.string
                .nrtc_setting_voe_audio_aec_key), 2 + ""));
        audioEffectAgcMode = Integer.parseInt(preferences.getString(context.getString(R.string
                .nrtc_setting_voe_audio_agc_key), 2 + ""));
        audioEffectNsMode = Integer.parseInt(preferences.getString(context.getString(R.string
                .nrtc_setting_voe_audio_ns_key), 2 + ""));
        ...
        videoMaxBitrate = Integer.parseInt(TextUtils.isDigitsOnly(value1) && !TextUtils.isEmpty(value1) ? value1 : 0
                + "");
        String value2 = preferences.getString(context.getString(R.string
                .nrtc_setting_other_device_default_rotation_key), 0 + "");
        deviceDefaultRotation = Integer.parseInt(TextUtils.isDigitsOnly(value2) && !TextUtils.isEmpty(value2) ?
                value2 : 0 + "");
        String value3 = preferences.getString(context.getString(R.string
                .nrtc_setting_other_device_rotation_fixed_offset_key), 0 + "");
        deviceRotationOffset = Integer.parseInt(TextUtils.isDigitsOnly(value3) && !TextUtils.isEmpty(value3) ? value3
                : 0 + "");
    }


    public void setBottomRoot(boolean isShow) {
        avChatVideo.setBottomRoot(isShow);
    }

    public void setDoctorVideoFinish(boolean isShow) {
        avChatVideo.setDoctorVideoFinish(isShow);
    }
    ...
    private void updateAVChatOptionalConfig() {
        ...
        avChatOptionalConfig.enableCallProximity(autoCallProximity).enableVideoCrop(videoAutoCrop).enableVideoRotate
                (videoAutoRotate).enableServerRecordAudio(serverRecordAudio).enableServerRecordVideo
                (serverRecordVideo).setDefaultFrontCamera(defaultFrontCamera).setVideoQuality(videoQuality)
                .enableVideoFpsReported(videoFpsReported).setDefaultDeviceRotation(deviceDefaultRotation)
    ...
    public boolean initiation() {
        ...
        if (clientType != null) {
            avChatVideo = new AVChatVideo(context, root.findViewById(R.id.avchat_video_layout), this, this, clientType);
        } else {
    ...
    public void setUserInfobean(UserInfoBean userInfobean) {
        avChatVideo.setUserInfoBean(userInfobean);
        this.userInfoBean = userInfobean;
        Log.i("tag00", "AVChatUI 用户信息：" + userInfobean.getHeadPortrait());
    }

    public void setOrderCode(String orderCode){
        avChatVideo.setOrderCode(orderCode);
        Log.i("tag00", "AVChatUI 订单code：" + orderCode);
    ...
    public void inComingCalling(AVChatData avChatData) {
        ...
        avChatVideo.setReceiverId(receiverId);
    ...
    public void outGoingCalling(String account, AVChatType callTypeEnum) {
        ...
        AVChatManager.getInstance().call(account, callTypeEnum, avChatOptionalConfig, notifyOption, new
     ...
     * 拨打音视频,医生端需要用户id
     */
    public void outGoingCalling(String account, AVChatType callTypeEnum, final int userId) {
        ...
        AVChatManager.getInstance().call(account, callTypeEnum, avChatOptionalConfig, notifyOption, new
            ...
            public void onFailed(int code) {
                ...
                if (code == ResponseCode.RES_FORBIDDEN) {
                    ...
                    if (SharedPreferencesUtils.getInt("callTimes") >= 3){
                        SharedPreferencesUtils.put("callTimes",0);
                        Log.i("tag00","三次了，记录未接通");
                        sendBroadCastAndSetTimes(userId);
                    }else {
                        Log.i("tag00","记录一次");
                        int temp = SharedPreferencesUtils.getInt("callTimes");
                        temp++;
                        SharedPreferencesUtils.put("callTimes",temp);
                    }
                    Toast.makeText(context, R.string.avchat_call_failed, Toast.LENGTH_SHORT).show();
                }
                closeSessions(-1);
    ...
    //设置未接听次数
    public void sendBroadCastAndSetTimes(int userId){
        if (clientType != null) {
            switch (clientType.getType()) {
                case 0:
                    Log.i("tag00","0 发送广播");
                    Intent mIntent1 = new Intent(getBroadCastStr(clientType));
                    mIntent1.putExtra("set_call_times",userId+"");
                    //发送广播,结束视频咨询
                ...
                case 2:
                    ...
                    mIntent.putExtra("set_call_times",userId+"");
                    //发送广播,结束视频咨询
    ...
    public void initAllSurfaceView(String largeAccount) {
        ...
        avChatSurface.initSmallSurfaceView(AppCache.getAccount());
    }
    ...
    public void onAudioToVideo() {
        ...
        avChatVideo.onAudioToVideo(AVChatManager.getInstance().isLocalAudioMuted(), AVChatManager.getInstance()
                .isLocalRecording(), recordWarning); // isMute是否处于静音状态
    ...
    public void onVideoToAudio() {
        ...
        avChatAudio.onVideoToAudio(AVChatManager.getInstance().isLocalAudioMuted(), AVChatManager.getInstance()
                .speakerEnabled(), AVChatManager.getInstance().isLocalRecording(), recordWarning);
    }
